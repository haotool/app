...
describe('getMetaTagsForRoute', () => {
  ...
  describe('當路由為首頁時', () => {
    ...
    it('應該包含正確的 canonical URL', () => {
      ...
      expect(result).toContain(
        '<link rel="canonical" href="https://app.haotool.org/quake-school/" />',
      );
    });
  ...
  describe('當路由為測驗頁時', () => {
    ...
    it('應該包含尾斜線的 canonical URL', () => {
      ...
      expect(result).toContain(
        '<link rel="canonical" href="https://app.haotool.org/quake-school/quiz/" />',
      );
    });
  });
  ...
  describe('SEO 最佳實踐驗證', () => {
    it('應該正確處理尾斜線一致性', () => {
      ...
      // 兩者都應該使用尾斜線
      expect(withSlash).toContain('quake-school/quiz/');
      expect(withoutSlash).toContain('quake-school/quiz/');
    });
