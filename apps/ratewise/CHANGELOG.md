...
- b72775e: 移除正式站上一份無人消費、停留在三個月前的歷史匯率檔。趨勢圖實際讀取的是每日更新的 CDN 資料，不受影響；此變更僅避免過期資料在網站上被誤取。

## 2.27.4

### Patch Changes

- 2d47aac: 新增來回換匯成本與計價基準判定的內部工具模組，供後續在幣別頁揭露「換出去再換回來」的實際損失與交叉換算警示。成本計算限定台銀牌告的報價單位，換錢所的反向報價會被單位保護擋下而不產生錯誤數字。本次尚未有使用者可見變更。

## 2.27.3

### Patch Changes

- 4891ad2: 趨勢圖不再於左上角常駐顯示「現金賣出走勢」標註，避免文字壓在走勢線上；價格基準資訊改由長按／滑過的 tooltip 呈現。

## 2.27.2

### Patch Changes

- d58e2d9: 修復收藏頁拖曳排序與下拉刷新互相干擾：拖曳排序中不再出現下拉刷新指示器或列表位移，放手也不會誤觸整頁重載；非收藏幣不再因拖曳被隱式加入收藏；TWD 列四語系顯示正確「基準幣」文案；下拉刷新指示器在瀏海機型不再被狀態列遮擋。
- 3d9f14a: 多幣別頁的現金／即期／換錢所切換改為 chip 樣式並將觸控目標擴大到 44×44px，手機上更容易發現與點按匯率類型切換；screen reader 不再將此切換誤唸為「已按下」
- fc14e16: 修復深色主題（Nitro、Racing）下 FAQ、關於、隱私等內容頁白字壓白底不可讀的問題，內容頁底色與文字現在正確跟隨所選主題
- 6cfc493: 修復深層頁面（設定、多幣別、幣別頁）冷開啟時偶發的首頁畫面一閃而過與底層渲染錯誤；邊緣快取異常回錯誤頁時自動以應用外殼接手，不再直接顯示 404 錯誤頁
- e0f506d: 可愛粉嫩與復古書卷主題的趨勢圖配色、Zen/Violet 的次要色與主題設定完全一致化
- 5e6c4dc: 幣別頁移除過期的固定匯率描述與無來源銀行宣稱，匯率數字改為每日即時資料驅動；About 頁常見問題移除與使用者無關的技術性條目，中間價差距說明全站統一
- 806484f: 修復冷啟動啟動畫面與設定頁「預覽啟動畫面」動畫不一致：冷啟動首屏即播放完整品牌動畫（雙圓彈入、雙色 wordmark、tagline），與預覽序列視覺一致；設定頁關閉啟動畫面後冷啟動也不再顯示；深色主題底色與 logo 兩處一致。
- cbc7111: 修復趨勢圖三項顯示缺陷：長按最新資料點的 tooltip 不再溢出畫面（含首尾資料點皆完整可見）；無即期幣別點擊「即期」時的原因說明氣泡完整顯示不被剪裁；趨勢圖與 tooltip 標註「現金賣出走勢」基準（四語系），避免與卡片即期價不一致造成誤解。

## 2.27.1

### Patch Changes

- f3e389f: 強化匯率資料可靠性：修正 30 天趨勢資料（回補 2026-06-30 至 07-03 因上游中斷寫入的過期快照為台銀真實歷史牌告），每日快照改以匯率管線存活狀態守門並加備援排程，休市期間趨勢圖不再誤判斷線也不再寫入錯誤的平線匯率；換錢所（明洞 MoneyBox）匯率停止更新超過一天會即時告警；台銀與換錢所匯率在寫入前新增異常數值熔斷，防止來源異常時顯示錯誤匯率
- e0d5b46: 離線兜底頁誠實重設計：改為誠實說明離線資料被系統清除的原因與恢復方式，在線時顯示「正在恢復應用程式」進度畫面，並直接顯示最後已知匯率唯讀快照（銀行賣出價優先），附診斷代碼供回報。視覺採全新韓系簡約扁平風格：移除所有漸層與裝飾陰影，全寬主按鈕、Toss 式匯率列表，7 主題（含深色）皆為扁平純色面。
- b27f4ed: 深色（科技感／賽車）與粉嫩、書卷、森林主題的卡片、警示、成功、收藏、高亮等色彩不再誤用預設淺色主題的顏色，深色主題下不再出現白底卡片或淺色區塊，各主題視覺一致性與文字對比全面修正。

## 2.27.0

### Minor Changes

- 9f19982: 新增 Racing 黑紅賽車主題（取代海洋深邃），自然森林改為白底韓系簡約風格

### Patch Changes

- c368fae: - 修復弱網或版本更新時誤入「離線頁」的問題：資源載入失敗不再清除離線快取，改為自動套用新版後重新載入
  - 消除「Unexpected Application Error / Load failed」錯誤畫面：全路由掛上品牌化錯誤防線，偵測到新版本時自動恢復，離線時顯示友善提示並於連線恢復後自動重試
- b591a3a: 啟動體驗優化：載入時優先顯示 HaoRate 品牌啟動畫面，應用就緒後淡出；同一瀏覽階段重新整理不重複顯示。
- a007188: 冷啟動時同步還原上次停留的多幣別模式，消除首頁先顯示單幣別再跳轉多幣別的可見閃爍
- 57a349d: 離線提示不再因網路瞬斷誤報，僅於確認離線時顯示
- 3d7cf68: 粉嫩／書卷／海洋／森林等主題在多幣別費率切換鈕現在正確顯示主題色，不再誤用預設品牌藍

## 2.26.1

### Patch Changes

- 44f1a8d: - 已安裝的 App 在啟動時偵測到新版本會自動載入更新，不再需要手動點擊「更新」按鈕（輸入中不打斷，非啟動時段仍以提示徵詢）
- d409f88: - 修復匯率停更：臺灣銀行網站啟用機器人驗證導致資料自 06/29 起未更新，抓取改以真瀏覽器完成驗證後下載，匯率恢復每 5 分鐘同步
...
- 5ef58cd: - 全新 HaoRate 品牌識別：App 圖示與 favicon 改為「雙幣交會」設計（實心本幣 × 鏤空外幣）
...
- 4df6cab: 修正開啟分享連結（帶 from/to/amount）進入單幣別頁時，會誤把「上次使用模式」偏好清成單幣別的問題；並讓舊版使用者的模式偏好在升級後正確保留。
- 6ed1bad: 修正在私密瀏覽模式或嵌入式瀏覽器中切換語言時，語言選擇可能無法套用的問題；改以實際偵測瀏覽器儲存可用性決定是否持久化語言偏好。
