...
describe('Hreflang Configuration (BDD)', () => {
  describe('🔴 RED: sitemap.xml Hreflang 配置', () => {
    // 優先讀取 public sitemap，避免舊 dist 產物污染測試結果。
    const publicSitemapPath = resolve(__dirname, '../public/sitemap.xml');
    const distSitemapPath = resolve(__dirname, '../dist/sitemap.xml');
    const sitemapPath = existsSync(publicSitemapPath) ? publicSitemapPath : distSitemapPath;
    const sitemapExists = existsSync(sitemapPath);

    // 檢查是否為正確的 ratewise sitemap（URL 應包含 /ratewise/）
    const isRatewiseBuild =
      sitemapExists && readFileSync(sitemapPath, 'utf-8').includes('haotool.org/ratewise/');
    ...
    it('should have correct xhtml:link elements if hreflang is present', () => {
      ...
      if (xlinkMatches) {
        // SEO_PATHS 22 條 * 2 語言 = 44（僅公開可索引路徑，不含 app-only 頁面）
        expect(xlinkMatches.length).toBe(44);
      } else {
