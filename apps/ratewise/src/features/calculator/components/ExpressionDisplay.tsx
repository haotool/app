...
export function ExpressionDisplay({ expression, result, error, preview }: ExpressionDisplayProps) {
  ...
  return (
    <div className="mb-6 space-y-2">
      ...
      <div className="relative min-h-[3rem] rounded-control bg-neutral-bg px-4 py-3">
        <div
      ...
      {result !== null && !error && (
        <motion.div
          className="rounded-control bg-primary-bg px-4 py-3"
      ...
      {error && (
        <motion.div
          className="rounded-control bg-danger-bg px-4 py-3"
          initial={{ opacity: 0, y: -10 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.15 }} // iOS 極速回饋：150ms
          role="alert"
