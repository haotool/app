...
/**
 * 精選色票（v3 緊湊化重校準）：單列 8 格＋「自訂…」進 react-colorful。
 * 校準規則（測試守門）：
 * - 與 7 個內建主題 primary 的 CIE76 ΔE*ab 全部 ≥ 20（內建近似色剔除——
 *   內建主題已可一鍵選用，精選票只提供內建沒有的色感）。
 * - 韓系飽和度、涵蓋色相環主要區段（靛藍→青→湖水→萊姆→琥珀→酒紅→紫紅→中性）。
 * - 全部色票對任一預設背景調 ≥ 2:1（不觸發近白/近黑 gate）。
 */
