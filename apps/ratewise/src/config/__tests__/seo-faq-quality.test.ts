 ...
 *
 * P12: FAQPage JSON-LD 選擇性啟用驗證
 *  - 高流量幣別（USD/JPY/KRW/EUR/HKD）應包含 FAQPage JSON-LD schema
 *  - FAQPage schema 應包含 mainEntity 陣列
 *  - 非高流量幣別暫不需要 FAQPage schema
 */
...
describe('P12: FAQPage JSON-LD 選擇性啟用（高流量幣別 AI/AEO 優化）', () => {
  const HIGH_TRAFFIC_CURRENCIES = ['USD', 'JPY', 'KRW', 'EUR', 'HKD'] as const;

  it.each(HIGH_TRAFFIC_CURRENCIES)('%s（高流量）正向頁應包含 FAQPage JSON-LD schema', (code) => {
  ...
  it.each(HIGH_TRAFFIC_CURRENCIES)(
    '%s FAQPage schema 應有 mainEntity 陣列且每項包含 Question/Answer',
    (code) => {
      const { jsonLd } = getCurrencyLandingPageContent(code);
      const faqPageSchema = jsonLd?.find((schema) => schema['@type'] === 'FAQPage');
      expect(faqPageSchema).toBeTruthy();
  ...
  it.each(HIGH_TRAFFIC_CURRENCIES)('%s FAQPage schema 應有 @context schema.org', (code) => {
    const { jsonLd } = getCurrencyLandingPageContent(code);
    const faqPageSchema = jsonLd?.find((schema) => schema['@type'] === 'FAQPage');
    expect(faqPageSchema?.['@context']).toBe('https://schema.org');
  });

  it('FAQPage schema 答案長度應 ≤ 800 字符（合理 AI 上下文長度）', () => {
    for (const code of HIGH_TRAFFIC_CURRENCIES) {
      const { jsonLd } = getCurrencyLandingPageContent(code);
      const faqPageSchema = jsonLd?.find((schema) => schema['@type'] === 'FAQPage');
