...
// schema.org: SpeakableSpecification 必須是 WebPage/Article 的 speakable 屬性值，
...
// 相容父實體優先順序：Article → TechArticle → HowTo → WebPage。
// 找不到時以 { '@type': 'WebPage', url } 作為後備容器。
const SPEAKABLE_PARENT_TYPES = ['Article', 'TechArticle', 'HowTo', 'WebPage'];
