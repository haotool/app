...
describe('convertCurrencyAmountWithMode — auto 模式（FROM用買入, TO用賣出）', () => {
  it('1000 USD→TWD auto：FROM=USD.buy=30.87, TO=TWD=1 → 30870', () => {
    // A→TWD（客戶賣外幣）：銀行以買入價收購 → amount * A.buy
    ...
    expect(result).toBeCloseTo(30870, 2);
  });

  it('1000 TWD→USD auto：FROM=TWD=1, TO=USD.sell=30.97 → 1000/30.97≈32.289', () => {
    // TWD→B（客戶買外幣）：銀行以賣出價賣出 → amount / B.sell（不同於 sell 模式的 amount / B.sell 相同值，此為語意一致）
    const result = convertCurrencyAmountWithMode(
      1000,
      'TWD',
      'USD',
    ...
    expect(result).toBeCloseTo(1000 / 30.97, 4);
  });

  it('1000 USD→JPY auto：FROM=USD.buy=30.87, TO=JPY.sell=0.204 → ≈151324', () => {
    // Cross rate（客戶賣 USD 買 JPY）：amount * (USD.buy / JPY.sell) = 1000 * (30.87 / 0.204)
    const result = convertCurrencyAmountWithMode(
      1000,
      'USD',
      'JPY',
    ...
    expect(result).toBeCloseTo((1000 * 30.87) / 0.204, 1);
  });

  it('auto 模式 USD→JPY 應比 sell 模式給更少 JPY（客戶面對買賣價差）', () => {
    ...
    // auto: 30.87/0.204≈151324 vs sell: 30.97/0.204≈151814（客戶面對 buy 與 sell 雙重價差）
...
describe('convertCurrencyAmountWithMode — mid 模式（中間價）', () => {
  ...
  it('三種模式 USD→JPY JPY 數量排序：auto < sell < mid', () => {
    // auto（客戶視角）: FROM 用 buy(30.87), TO 用 sell(0.204) → 雙向不利價差，JPY 最少
    // sell: FROM 用 sell(30.97), TO 用 sell(0.204) → 僅 FROM 有利，JPY 居中
    // mid: FROM 用 mid(30.92), TO 用 mid(0.202) → JPY.mid 低於 sell，TWD 兌換 JPY 更多
    const autoResult = convertCurrencyAmountWithMode(
      1000,
      'USD',
      'JPY',
