...
export default function Favorites() {
  ...
  // 非收藏段起點（index 0 = TWD，其後為收藏幣）；用於顯示「先收藏才能排序」分段提示。
  const firstNonFavoriteIndex = 1 + favorites.filter((code) => code !== 'TWD').length;
                ...
                {(provided: DroppableProvided, snapshot: DroppableStateSnapshot) => (
                  ...
                  >
                    {allCurrencies.map((code, index) => {
                      ...
                      return (
                        <Fragment key={code}>
                          {/* 非收藏段分段提示：一行 muted 小字，說明先收藏才能排序。 */}
                          {index === firstNonFavoriteIndex && (
                            <p
                              className="text-2xs opacity-40 font-medium px-2 pt-4 pb-1"
                              data-testid="favorites-star-to-reorder-hint"
                            >
                              {t('favorites.starToReorderHint')}
                            </p>
                          )}
                          <Draggable draggableId={code} index={index} isDragDisabled={!canDrag}>
