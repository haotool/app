...
const FAQ_ENTRIES = [
  {
    question: '紐元匯率數據來源與更新頻率？',
    answer:
      '資料 100% 參考臺灣銀行牌告匯率（現金/即期買入賣出價），每 5 分鐘自動同步一次，首頁會顯示最近更新時間。',
  },
  {
    question: '紐元換算結果與實際交易是否相同？',
    answer:
      'RateWise 提供牌告參考匯率，實際交易匯率可能因銀行或線上換匯平台產生 0.5%~2% 價差，請以交易方公告為準。',
  },
  {
    question: '去紐西蘭旅遊要換多少紐元？',
    answer:
      '建議使用 RateWise 換算器輸入預算金額，即可得知台幣對應紐元。一般建議行程預算 + 20% 餘裕，並參考當日牌告匯率。',
  },
  {
    question: '紐元匯率走勢如何查看？',
    answer: '首頁點擊紐元卡片可展開 7~30 天歷史趨勢圖，視覺化呈現匯率走勢，幫助判斷換匯時機。',
  },
];
...
const HOW_TO_STEPS = [
  {
    position: 1,
    name: '選擇紐元 (NZD)',
    text: '在首頁將「從」設定為 NZD，或在多幣別模式中找到紐元卡片。',
  },
  {
    position: 2,
    name: '輸入紐元金額',
    text: '輸入 NZD 金額後即時計算對 TWD 的換算結果；多幣別模式可同時查看 30+ 貨幣。',
  },
  {
    position: 3,
    name: '查看歷史趨勢',
...
export default function NZDToTWD() {
  return (
    <>
      <SEOHelmet
        title="NZD 對 TWD 匯率換算器 | 即時紐元台幣匯率"
        description="NZD 對 TWD 即時匯率換算，參考臺灣銀行牌告匯率，每 5 分鐘更新。紐元換台幣、紐西蘭旅遊換匯必備工具，支援現金/即期匯率、離線 PWA、多幣別同時換算。"
        pathname="/nzd-twd"
        canonical="https://app.haotool.org/ratewise/nzd-twd/"
        keywords={[
          'NZD TWD 匯率',
          '紐元換台幣',
          '紐元匯率',
        ...
        breadcrumb={[
          ...
          { name: 'NZD → TWD 匯率', item: '/nzd-twd/' },
        ]}
        faq={FAQ_ENTRIES}
        howTo={{
          name: '如何查看 NZD 對 TWD 匯率',
          description: '使用 RateWise 3 步驟快速換算紐元對台幣，並查看歷史趨勢與多幣別。',
          steps: HOW_TO_STEPS,
          totalTime: 'PT30S',
        }}
      />

      <main className="min-h-screen bg-gradient-to-br from-neutral-light via-emerald-50 to-white">
        <div className="container mx-auto px-4 py-10 max-w-5xl">
          {/* Breadcrumb Navigation */}
          <Breadcrumb
            items={[
              { label: '首頁', href: '/' },
              { label: 'NZD → TWD', href: '/nzd-twd/' },
            ]}
          />
          ...
          <div className="flex items-center justify-between gap-4 mb-6">
            <div>
              ...
              <p className="text-neutral-text-secondary mt-2">
                即時紐元對台幣匯率，參考臺灣銀行牌告，每 5
                分鐘更新。紐西蘭旅遊換匯必備工具，支援現金/即期匯率、離線 PWA、多幣別模式。
              </p>
            </div>
            <Link
              to="/"
          ...
          <div className="grid md:grid-cols-2 gap-6 mb-8">
            <div className="bg-white rounded-2xl shadow p-5 border border-neutral-dark">
              <h2 className="text-xl font-semibold text-neutral-text mb-3">🇳🇿 紐元匯率快速重點</h2>
              <ul className="space-y-3 text-neutral-text">
                ...
                <li className="flex items-start gap-2">
                  <span className="text-emerald-600 mt-0.5">•</span>
                  <span>紐西蘭旅遊：奧克蘭、皇后鎮旅費換算一鍵完成。</span>
                </li>
                <li className="flex items-start gap-2">
                  <span className="text-emerald-600 mt-0.5">•</span>
            ...
            <div className="bg-emerald-600 text-white rounded-2xl shadow p-5">
          ...
          <section className="bg-white rounded-2xl shadow p-6 border border-neutral-dark">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h2 className="text-2xl font-bold text-neutral-text">紐元換匯常見問題</h2>
