 ...
 * - 冷快取（html-cache miss）：先向 origin 取該 URL 的 per-route SSG HTML，避免誤回傳
 *   index.html 首頁快照造成 React #418 hydration mismatch（各路由有獨立預渲染 HTML）
 * - 網路失敗或 timeout：才 fallback 到 precache index.html（SPA shell）；
 *   precache 也 miss（iOS eviction）時走 offline.html 兜底鏈
 *
...
async function handleNavigationRequest({
  ...
  // 冷快取：先取正確的 per-route SSG HTML（8s bounded），避免回首頁快照造成 #418。
