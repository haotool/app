...
## 1000x × PPR 泡泡幣組合（設計內行為，不另加攔截）

- PPR 合成行情的單 tick 基礎波動為 ±0.1–0.5%（`PPR_BASE_STEP_MIN`／`PPR_BASE_STEP_MAX`），
  已大於 1000x 的強平緩衝 0.05%（強平距離 `entry × (1/lev − mmr)` 於 1000x 時為開倉價之 0.05%）。
- 亦即極高槓桿＋迷因幣的組合「幾乎必然快速強平」——這是設計內的教育示範：
  本產品為模擬環境、無真實資金，讓使用者以零成本親身體驗極端槓桿疊加極端波動的脆弱性，
  正是模擬交易所的核心價值之一。
- 因此不對此組合另加下單攔截或槓桿上限特判；風險語義由既有 >100x warning 提示與
  PPR 常駐「虛構迷因幣」標示共同傳達。
