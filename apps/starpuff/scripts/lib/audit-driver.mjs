...
export function installAuditDriver(opts) {
  const {
    ...
    // §111 walk transform probe：同系供給怪集合——獵集吸食路徑（非 grantStar 注入）。
    huntKinds = [],
  ...
  const d = {
    m: {
      ...
      shots: 0,
      samples: [],
      // 變身優勢 hook（#816 W2）：成功變身次數；§111 首次變身時刻（probe 量測）。
      transforms: 0,
      firstTransformMs: -1,
    },
  ...
  const tick = () => {
    ...
    if (kind === 'boss') {
      ...
      if (dodge) {
        ...
        if (flap) {
          ...
          if (wall) {
            // 對穿路徑常含核心本體（牆自場緣掃入、核居中）——垂直延遲沿跨核
            // 幾何 730ms，防低空平移削核（W1.6 四測 p4 本體接觸 ×82 主因）。
            hop(Math.sign(wall.x - s.px) || 1, 3, 730);
            return;
          }
        }
    ...
    // §111 walk transform probe：獵集同系味→SP 變身（模擬「想變身的玩家」策略）。
