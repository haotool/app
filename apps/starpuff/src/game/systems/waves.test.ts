...
describe('情境提示呈現責任', () => {
  it('WaveRunner 不再建立遮擋遊戲畫面的中央教學浮字', () => {
    const text = vi.fn(() => chainable());
    const scene = {
      events: { on: vi.fn(), off: vi.fn(), emit: vi.fn() },
      cameras: { main: { scrollX: 0 } },
      scale: { width: 854, height: 480 },
    ...
    const runner = createWaveRunner(scene, enemies, 1);
    runner.start();
    runner.update(1000);
...
describe('滿潮救援上台錨定（#841）', () => {
  ...
  it('滿潮期救援於最近平台頂生成 spora 並關閉重力', () => {
    ...
    const runner = createWaveRunner(scene, enemies, 14, { holdSpawn: () => true });
    runner.start();
    runner.update(4000);
    expect(spawns).toHaveLength(1);
    expect(spawns[0]?.kind).toBe('spora');
    // L14 平台 x∈{420,820,1250,...}：距玩家 1000 最近＝820（y=272）；
    // 站位 y＝平台頂（272-8）− 敵體半高。
    expect(spawns[0]?.x).toBe(820);
