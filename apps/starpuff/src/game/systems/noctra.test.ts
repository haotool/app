...
// 彈幕池瞬時旗標循環（§119 潮環，PR #886 R3）：noctra 投彈被潮環撥開／殼化
// 反彈後回池，復用時旗標必須歸位（acquirePooled 取出即復位），否則潮化對回收
// 彈幕靜默免疫。本檔 harness 僅承載池循環守門，完整 FSM 行為由 noctraFsm 單測覆蓋。
...
describe('Noctra 彈幕池瞬時旗標循環（§119 潮環，PR #886 R3）', () => {
  beforeEach(() => {
    let seed = 7;
  ...
  it('撥開/反彈旗標殘留的投彈回池復用後必為 false', () => {
    ...
    // 下一輪彈幕復用同一物件：旗標必須歸位，否則潮化對回收彈幕靜默免疫。
