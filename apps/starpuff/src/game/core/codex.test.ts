...
describe('CODEX_MONSTERS', () => {
  it('收錄全部三十三種小怪與十魔王（§120/§123/§126 十五新怪＋§122/§123/§126 五新王入鑑）', () => {
  ...
  it('立繪鍵對應資產註冊表或 §120 佔位鍵（素材車交付後 pending 清空）', () => {
    const registered = new Set(ASSETS.map((asset) => asset.key));
    for (const monster of CODEX_MONSTERS) {
      expect(
        registered.has(monster.textureKey) || PENDING_TEXTURE_KEYS.includes(monster.textureKey),
      ).toBe(true);
    }
  });
  ...
  it('可吸標記與戰鬥規則一致（§5/§16/§30/§47/§52/§59/§73/§80/§120/§123），四種條件可吸', () => {
    ...
    expect(CODEX_MONSTERS.find((m) => m.kind === 'twinkla')?.conditional).toBe(true);
    expect(CODEX_MONSTERS.filter((m) => m.conditional)).toHaveLength(4);
  });
