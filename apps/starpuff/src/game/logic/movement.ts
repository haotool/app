...
export function approachVelocity(
  ...
  // 超速殘速（擊退）先夾回速帽帶，避免殘速滑行（v5 為逐幀瞬時覆寫）。
  const cap = Math.max(PLAYER.moveSpeed, Math.abs(target));
  const capped = Math.abs(current) > cap ? Math.sign(current) * cap : current;
  const dt = deltaMs / 1000;
  if (target === 0) {
    const step = PLAYER.decelPxPerSec2 * rateMul * dt;
