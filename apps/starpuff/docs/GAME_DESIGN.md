...
- 三屬性沿用 §20；新增連鎖規則：
  ...
  - 彈匣全滿長按 B 0.8s：星暴大招——清場全部小怪 + 魔王 12 傷，震屏 + 白閃 + 星雨粒子，清空彈匣。（v9.1 已由 §64.2 補訂：發動附 5s 無敵窗，與受擊 i-frame 取較大值不疊加。）（v19 已由 §109 取代：滿匣自動結晶＋SP 鍵手動引爆，B 長按星暴路徑退場。）
...
- 殼盾（防禦原型）：頂槽殼盾星且未滿匣時，長按 B ≥150ms 舉正面護盾（取代吸入語意；滿匣長按維持星暴優先，肌肉記憶不變）。格擋彈幕/接觸一次：不掉血不擊退、消耗頂槽、觸發反擊星爆（盾前 90px 傷害 3）＋短無敵 0.8s，冷卻 4s。正面判定＝傷害來源位於面向側（同 x 視為正面）。點按仍為發射。（v19 已由 §109 補訂：長按星暴退場，頂槽殼盾星即成立殼盾情境——滿匣不再讓位。）
...
- 觸發：彈匣內同系可變身星彈合計 ≥3 發（強化槽為連吞兩發合成、計 2 發；金星/混合槽
  ...
  （v19 已由 §109 取代觸發面：改 SP 鍵點按立即變身／再按提前解除，0.6s 長按門檻
  廢除；資格條件與形態語彙不變。）
