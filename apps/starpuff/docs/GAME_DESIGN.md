...
- bot 前後（forage 錨定量測：x=1400 定點吃怪換血、滿匣傾瀉、精英隔離，300s）：
  最長彈藥斷檔中位數 舊 mix 9.0s（5 輪：9／10.1／5.4／1.0／229.5）→ 新 mix
  3.4s（3 輪：2.9／3.4／193.6），優於 L6 鄰關對照 29.9s——issue 驗收
  「回落至鄰關水準」成立；idle 中場站定複測 reach 真空最長 1.8s（<30s 門檻）。
- 殘留邊界（誠實記錄）：兩側各有零星爆炸輪（229.5s／193.6s）——同屏 5 席被
  「名義可吸但不可及」個體（高空 gusty／遠端 spora）佔滿且 bot 不跳吸、不下砸、
  不死亡時的凍結組態。真人玩家走近 gusty 觸發域即誘導俯衝、走向 spora 即可吸，
  屬 bot 固定策略盲區；系統面已由 §107.1 救援律兜底（僅在「同屏零可吸」成立時）。
  mix 內合法調參空間已用盡（gusty 主場 ≥25% 契約），若需再收斂需 PM 裁決
  放寬主場契約或引入駐留型個體同屏壽命機制。
