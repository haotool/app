...
- 修法（waves.ts＋levels.ts 最小增量，不動 §107.1 觸發閾值）：
  - 生成位：玩家前方 160–240px（`rescueSpawnX`，越界/尾端安全區改後方等距）；
    懸浮定高品種降至站立錐可及帶 y=300；潮汐上收沿 adjustSpawn（同層可達）。
  - 品種：恆可吸 ∩ 低威脅（safe）優先（`pickRescueKind`）；滿潮排除位移壓迫型
    （gusty 側風）；無 safe 候選回落恆可吸全集（anti-softlock 優先）。
  - 單席＋逾時重定位：場上救援個體上限 1；8s 未取得重定位到玩家身旁。
  - 飢荒判定改可及性口徑：走動關僅計「近域 400px 內非威脅型」可吸個體——
    視野外或 ranged 威脅型（放電/反射/拋射、零彈追擊高成本）不阻斷救援計時；
    魔王關單屏維持全域計數。
- 工具實測（starburst 探針、mid bot、90 樣本）：45s 未恢復 16→0（爆炸輪根除）、
  p95 19.9→13.4s、p50 5.5→5.4s。p95 門檻 10s 未全達：殘差離群（每關 ≤1 樣本
  10–21s）歸因 bot 零彈期死亡重生循環與高空 floaty 跳追執行力，非救援機制失效
  （驗屍幀存 `.claude/product-intel/level-audits/`）。
