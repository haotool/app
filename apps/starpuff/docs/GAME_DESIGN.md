...
> 版本：v8.0（PM 親撰；v8 世界擴張 §50-§56——L5-L7 三新關/上升氣流/新怪 ×3/星彈九系/第二魔王 Noctra、v7 手感與深度 §44-§49、v6 存檔/世界地圖/新技能/手感 §38-§43、v5 控制自訂/暫停/圖鑑/開場 §33-§37、v4 免轉向與元素包 §28-§32、v3 橫式轉向 §21-§27）｜路由：`https://app.haotool.org/starpuff/`｜24h 衝刺交付

## 1. 產品定位

- 橫式（landscape）四關側向卷軸動作遊戲：三關走動探索 + 魔王關（Boss Rush 收尾），單局 2–4 分鐘，可無限重玩。（v8 已由 §50 取代：七關雙魔王——五關走動探索＋兩座魔王關。）
...
`BootScene`（載入+進度條）→ `TitleScene`（標題、開始按鈕、音效提示）→ `MapScene`（v6 世界地圖 hub，§39）→ `GameScene`（四關制）（v8 已由 §50 取代：七關流程）→ `ResultScene`（勝/敗）。
...
- 圖鑑/技能介紹（`CodexScene` 單場景雙分頁，資料 SSOT `core/codex.ts`，立繪一律取既有 sprite 資產、禁止新美術）：
  - 圖鑑分頁：全 8 角色（jelly/floaty/spiky/puffy/chompy/shelly/zappy/boss）4×2 網格——立繪＋名稱＋行為一句話＋可吸/不可吸圓點標記。（v7 擴 5×2 十格；v8 已由 §52 取代：7×2 十四格。）
...
流程擴為：L1-L3 → L4 果凍王 → L5-L6 → L7 暗月蝠王（第二魔王收尾）。解鎖鏈沿 §39
...
FSM 指令帶出、GameScene 依現量夾限走正式 spawn 管線）`
...
- 盤旋駕駛：常態沿視寬正弦盤旋（高度 168±14 呼吸浮動）；dive/sweep tween 接管期間讓位
  （steering 互斥），死亡冪等（FSM defeated 單向鎖存，演出僅執行一次）。
