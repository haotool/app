...
>
> **星暴完整規格＝§109.3**（`01-controls-input.md`）：滿 5 槽按 SP 手動結晶為蓄能星、再按 SP 引爆、跨關持有與死亡清除。本檔 §23／§40 只寫到與星彈系統交界的部分；v19 前的 B 長按觸發面見各該節的已廢止附註。
>
...
- 三屬性沿用 §20；新增連鎖規則：
  ...
  - 彈匣滿 5 槽時按 SP **手動結晶**為蓄能星，再按 SP 引爆星暴——清場全部小怪 + 魔王傷害依投入星值計價（`starstormBossDamage`，保底 12） + 5s 無敵窗（與受擊 i-frame 取較大值不疊加，§64.2），震屏 + 白閃 + 星雨粒子。完整規格見 §109.3。
...
| 吞下   | 星彈屬性 | 效果                                                                    | 視覺                        |
| ------ | -------- | ----------------------------------------------------------------------- | --------------------------- |
| shelly | 殼盾星   | 直射傷害 5、速度 480、穿透 0；吞下即披龜甲護甲（見下）                  | 青綠 tint #7FD8C8           |
| zappy  | 雷鏈星   | 直射傷害 5、速度 585、穿透 0；命中後跳電至 160px 內最近 2 敵、各 3 電傷 | 電黃 tint #FFE28A、折線閃電 |

- 龜甲護甲（防禦原型，原殼盾重設計）：**吞下殼殼即被動披甲 20 秒**，不需按鍵、不佔彈匣頂槽。視窗內自動格擋彈幕/接觸一次：不掉血不擊退、觸發反擊星爆（身周 90px 傷害 3）＋短無敵 0.8s，擋下即耗盡。**不分正背面**（全向）；重複吞下重置視窗而不疊層。B 鍵因此回到兩語意（點按發射／長按吸入），披甲期間吸吐與發射照常。視覺為綠色龜甲防護罩，吸吐脈動，剩餘 <3s 閃爍預告卸甲。
...
- 平衡註記：龜甲護甲一次一甲、不疊層，且需重新吸一隻殼殼才能再披，防無限格擋；雷鏈單跳 3 傷不足以秒 chompy（10 HP），保持坦怪定位；§24 連吞彩蛋序列不含新屬性（zappy 現記為 zappy，不再以 floaty 代位計序）。
...
- 觸發：彈匣內同系可變身星彈合計 ≥3 發（強化槽為連吞兩發合成、計 2 發；金星/混合槽
  破壞資格）時 HUD 彈藥列脈動亮起變身提示，變身鍵浮現；按變身鍵**點按**
...
- L3 變身首教位點：進關前段（x 460–600）drillSpawns 保證 3 隻 floaty（同系→風化）；
  變身鍵資格徽章首次浮現時一次性教學浮字「同系星彈 ×3！按變身鍵立即變身」（`playerFeel.ts` 變身首教浮字，session 慣例）。
...
（同味計數 ≥3，不限地面）；anti-softlock 紅線不變——全部為優勢解非必需解。
