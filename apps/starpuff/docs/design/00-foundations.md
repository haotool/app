...
- 橫式（landscape）側向卷軸動作遊戲：**十區三十關**——二十關走動探索＋十座魔王關（各王另有 EX 變體），單關 2–4 分鐘，可無限重玩。關卡編成見 §84。
...
`BootScene`（載入+進度條）→ `TitleScene`（標題、開始按鈕、設定入口）→ `MapScene`（世界地圖 hub，十區分頁，§39／§78）→ `GameScene`（三十關，§84）→ `ResultScene`（勝/敗）；L30 全破先經 `CreditsScene` 謝幕再進結算（§84）。
...
型別檢查與測試守門**——契約改了它不會編譯失敗，只會安靜地量出假數據。

假數據比沒有數據更危險：它看起來像結論，會被寫進報告、當成調整平衡的依據。

**因此變更下列任一項時，audit driver 必須與文件、e2e 同批更新並列入 PR 檢查：**

- 按鍵映射與輸入語意（哪顆鍵、點按或長按、節流語意）
- 技能／形態的觸發條件與資格判定
- 資源取得節奏（彈匣、蓄能、補給）中任何由「自動」改為「玩家觸發」的項目

最後一項最容易漏：自動行為在 driver 裡通常沒有對應程式碼，改手動後需要**新增**操作
邏輯，而非修改既有邏輯——沒有既存呼叫點可循，純靠人記得。

**實例（三次連續失誤，同一根因）**：`#952` 拆鍵後 driver 仍按舊鍵；補上後暴露「補星
與變身互鎖」的死迴圈使形態 uptime 逼近 100%；再修後又發現結晶與引爆共用節流、且走動關
變身鉤子仍按舊鍵。三次皆非產品缺陷，全是量測工具與契約脫節。

**判讀紀律**：難度數據異常時，先確認 driver 是否仍與當前契約一致，再談平衡。以
`transforms: 0`、`hitRate` 驟降、`TTK` 從未達成等「行為完全消失」型訊號為警訊——
真實平衡問題通常是連續偏移，而非歸零。
...
### 13.1 作品說明與致敬聲明（2026-08-26）

- 作品說明、致敬定位、教學素材製作說明與虛構內容免責文字由 `core/legalCopy.ts` 統一管理，於設定頁 `details` 漸進揭露，不阻塞首次操作。
- 對經典平台遊戲的吸入、變身與平台動作互動語彙僅作原創設計致意；本作不隸屬、未獲授權、未受贊助於任何第三方品牌、角色或權利人，也不使用其角色、商標或素材。
- 遊戲中的市場、價格、事件與數值皆為虛構玩法內容，不構成投資、交易或其他專業建議；教學插圖由生成式工具輔助製作並經人工檢視。
