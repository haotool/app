...
> **職責範圍**：玩家怎麼操作遊戲。搖桿與 A/B/SP 鍵語意、跳躍與下衝擊輸入矩陣、觸控寬容度與真實手勢判定、走動與移動手感、直持旋轉殼與座標換算，以及虛擬鍵布局 SSOT 與按鈕配置頁。
>
> **不在本檔**：按下之後發生什麼事——星彈與技能效果見 `02-combat-star.md`；暫停選單、主選單、HUD 命中層見 `06-ui-shell-pwa.md`；偏好如何持久化見 `05-save-settings.md`。
>
