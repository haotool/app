...
> **職責範圍**：玩家怎麼操作遊戲，**以及與輸入直接連動的彈匣／蓄能狀態機**。搖桿與 A/B/SP 鍵語意、跳躍與下衝擊輸入矩陣、觸控寬容度與真實手勢判定、走動與移動手感、直持旋轉殼與座標換算、虛擬鍵布局 SSOT 與按鈕配置頁，以及星暴 2.0 的滿匣手動結晶與蓄能星生命週期（§109.3）。
>
...
- 練習區若玩家失手，會自動補回仍可到達的星星、Shelly 或三顆同味星；下砸目標失去可達性時
  只重置目前步驟。未完成前只提供「再試一次」，不提供可跳過的「下一步」，避免卡在半套教學。
