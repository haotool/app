...
> **職責範圍**：玩家怎麼操作遊戲，**以及與輸入直接連動的彈匣／蓄能狀態機**。搖桿與跳躍／星形／SP 鍵語意、跳躍與下衝擊輸入矩陣、觸控寬容度與真實手勢判定、走動與移動手感、直持旋轉殼與座標換算、虛擬鍵布局 SSOT 與按鈕配置頁，以及星暴 2.0 的滿匣手動結晶與蓄能星生命週期（§109.3）。程式仍保留桌機鍵位與歷史 A/B 相容命名，但行動教學以按鈕顏色與形狀為準。
>
...
- 右側：薄荷綠上箭頭圓鍵（跳躍）與珊瑚粉星形圓鍵（吸入／吐出；位置可自訂，§34；技能可用時其上方浮現金色 SP 情境鍵，§109.2）——跳躍鍵空中連按＝拍翅漂浮，最多 3 次，落地重置；星形鍵吸／射（三語意見 §109.1）。
- 正常關卡採情境式小天使提示：只在關卡首次教到新機制、或實際遇到對應情境時顯示一張
  不阻塞操作的小卡；玩家可關閉，之後也能從設定重新開啟。完整實作練習只在首次選擇
  「進入練習區」或設定重播時出現，不以第一關一次塞完所有規則。按鈕位置可由設定→按鈕
  配置調整。
...
- 首次按「開始遊戲」先選「進入練習區」或「直接開始」。直接開始不顯示完整教學卡，僅由
  情境小天使在對應關卡提示；練習區則由真正的 `GameScene` 輸入、物理與事件管線逐步實作。
...
- 吸入步驟明確示範「右手食指長按珊瑚粉星形鈕／鍵盤 X」可連續吸入靠近的多顆星星；不需要快速連點。
  右手大拇指按薄荷綠上箭頭鈕／鍵盤 Z，可在食指持續按住星形鈕時同時跳躍。兩個觸控鈕各自以獨立 `pointerId` 維持
  `held` 狀態，放開其中一指不會誤放另一鍵；桌機鍵盤 Z＋X 具有等價行為。
- 練習區若玩家失手，會自動補回仍可到達的星星、Shelly 或三顆同味星；下砸目標失去可達性時
  只重置目前步驟。未完成前只提供小型「重試本步」工具，不提供可跳過的「下一步」，避免卡在半套教學。
...
- **drop-intent 緩衝窗**：`advanceDownBuffer` 純函式維護 450ms 窗——down 釋放後
  ...
  幀對齊與第二指命中保留餘裕；搖桿已達下向扇區後即使 `pointerup` 先於下一個遊戲幀，
  仍由事件邊緣保留一次 drop intent 才啟動緩衝，不把真實 flick 變成普通跳。空中下砸
...
- 直持引導：尚未觀測到橫持時以 body-level `orientation-coachmark` 非模態提示「建議橫持遊玩」，
  以手機 90° 反轉動畫示範解除方向鎖定與橫持；提示層不使用 `aria-modal`，不攔截遊戲 pointer。若瀏覽器提供
  `screen.orientation.unlock()`，按鈕只做 best-effort 嘗試；否則明確引導玩家到系統控制項手動解除。
  runtime 監聽 resize／orientationchange／MediaQuery orientation，使用者實際轉橫即收卡並記憶
  `sp-orientation-landscape-seen`；按掉卡片但仍直持時，重新進站仍提示。舊 `sp-orientation-hint`
  僅清理、不再抑制流程；遊戲不強制鎖定方向，直持仍保留 CSS 旋轉殼；觸控裝置（含觸控筆電）
  恆不進桌機模式，旋轉殼語意保留。
- e2e（t2.spec.ts 三情境 × 三 project）：桌機（殼 transform none、#controls 隱藏、鍵位卡一次性、
  鍵盤可操作）／直持未轉橫再次提示／橫持（方向小天使不重複出現）；coachmark AABB 與截圖驗收
  另見 `modal-landscape.spec.ts`。
...
- 語意（天然互斥，`resolveSpPress`；圖示即行為，`resolveSpMode` 同序）：
  ...
  2. 蓄能星存在 → 點按＝引爆星暴。
  3. 變身移至獨立 TF 鍵（#952）：資格成立（同味計數 ≥3，不限地面）→ 點按＝立即變身。
- 鍵盤映射：`C`＝SP（`Z` 跳、`X` 吸/射不變）；§89 桌機鍵位卡與 Title 操作說明
  同步標示。

### 109.3 星暴 2.0——蓄能結晶＋手動引爆

- 彈匣擴為 5 槽（`STAR.maxAmmo`）；滿 5 槽時按 SP **手動結晶**（#954）——彈匣清空、頭頂
