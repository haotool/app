...
> **職責範圍**：30 關的資料與節奏。關卡資料契約與各關配置、十區分頁世界地圖與解鎖鏈、彩蛋、平台與環境機制（單向平台／氣流／噴口／潮汐／折躍／低重力／流星雨）、卡點重生、魔王關前室與增益體系、每關攻略 PoC 與命名層級。
>
...
- 正式關卡不再顯示中央教學浮字：`guidanceDirector` 只依 `core/learning.ts` 的單一 lesson registry
  ...
  則在對應關卡情境化出現；文案、素材、focus selector 與行動版按鈕 token 不得在 director 另存第二份。
- 完成基本操作後只提供「開始正式 L1」，進入 L1 後由上述情境提示補充規則，避免首次 onboarding
  變成長篇百科。練習區目標失手、死亡或離開可自動補回／重置目前目標，也可按小型「重試本步」；
  救援不會把半完成操作誤算成功。

#### 15.2.1 行動裝置顯示規則（2026-08-26）

- 行動版提示以控制器外觀命名：薄荷綠上箭頭＝跳躍、珊瑚粉星形＝吸入／吐出、金色星化 glyph＝變身／星暴；右手食指長按星形鈕可連吞，拇指可同時按上箭頭鈕。
- 提示卡由 `learningCoachmark.ts` 以真實 viewport 幾何避讓 `#joy-zone` 與所有虛擬鍵；直持放在搖桿上方、橫持放在左半屏外的安全帶。每一步仍必須由正式事件與碰撞管線驗收，補怪只恢復機會不跳過操作。
...
- L8 開場 hint 浮字（LevelSpec.hint 資料驅動，沿教學浮字機制）：首次星化教學
  「同系星彈集滿 3 發，按變身鍵星化（空中也可以）」（`levels.ts` L8 hint）；起手區前
  500px 無精英無破磚（安全教學帶）。
...
- L30 崩盤王座：最終魔王關（魔王規格見 §126.2）；魔王關體系沿 §69（前室 400px
  ＋護盾/星力二選一＋關門後 7000ms 讀招保護＋P2 護盾泡）；補生全可吸、zappy 列首
  （輪替恆含雷味）；
  彩蛋 crown-early-hit（可擊打後 5s 內首擊，與 L28 同型）。arena 幾何收斂為
  `L30_ARENA_PLATFORMS` 單一中央低台（`offsetPx=0`、`y=256`、`w=280`）；左右
  平台移除，地面走位與中央台讀招換位為兩條主解。
...
- 實測（Chromium、`scripts/level-audit.mjs`、5 局、cap 90s）：L29 歷史序列為 mid
  6/7（86%）；2026-08-02 L30 無變身 high 5/5（100%，TTK 平均 36.2s、通關平均
  39.0s、0 死亡）、mid 2/5（40%，TTK 平均 65.6s、通關平均 68.4s、0.4 死/run）；
  引力化 high 5/5（100%，TTK 平均 39.9s、通關平均 42.7s、2.6 次變身/run、0 死亡）、
  mid 4/5（80%，TTK 平均 50.4s、通關平均 53.3s、3.6 次變身/run、0.2 死/run）。
  四組 console error 皆為 0；變身使 mid 通關率由 40% 提升至 80%，high 維持全通關並
  提供防守容錯。殼殼反制探針（L1、Chromium、100 次正確時機＋30 次旋轉期）亦達標：
  暈眩窗成功吸入 79%、旋轉期 0%、console error 0。後續若調整 L30 FSM 或供給，必須
  以同口徑重跑後更新本段。
