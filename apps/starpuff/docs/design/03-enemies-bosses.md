...
- **L30 幾何與接觸安全 SSOT**：arena 只保留中央一塊較低單向平台（`offsetPx=0`、
  `y=248`、`w=280`，平台頂 240）；左右平台移除，地面仍是主路線。劉董物理箱寬高
...
- **招牌全屏下跌箭頭 arrowrain（P2）**：批次落下（慢大 → 快小；EX 加第三批
  ...
  中央崩跌衝擊波與雙向地面波。硬規則全數落地：每批缺口由 `LIUDONG.safeLaneCount=5`
  控制且恆開、錨定玩家近旁五車道（僅保留一條危險車道作為讀招與輸出取捨；
  全域隨機缺口在 650ms 陰影窗內跨不了 3 車道＝不公平死局，實測回調）、
  陰影預警 650ms ≥600ms。
- **P2 招池**：arrowrain／klinewave（K 線海嘯：柱列單側掃場、留玩家近旁五車道缺口）／
...
- **基礎星彈 anti-softlock 契約（全遊戲收官驗收）**：全招式均有純走位解
  ...
  替代路線，不得用舊報告數字代替當前證據。最新同口徑結果：無變身 high 4/5（80%）、
  引力化 high 5/5（100%）、mid 2/5（40%），三組 console error 皆為 0；引力化
  通關率與死亡率改善，但 TTK 不固定宣稱提升。
