...
> **職責範圍**：場上會動的東西。主角與小怪的設定與 FSM、精英變體系統、十座魔王（Jellord／Noctra／Prismix／Syrona／Voidra＋星海終局篇 Tariffang／Maridella／Reflector／Gravion／Liudong）的階段結構、加權選招與主題招式、EX 變體與 P4 第二血條，以及魔王演出與難度根修紀錄。
>
...
- 新怪物 ×2（全原創）：
  - 殼殼 Shelly：藍綠硬殼龜果凍。第一發星彈 → 縮殼短衝 0.9s（期間無敵、傷害 1）→ 停下暈眩 2.2s（可吸/可擊殺）。HP 2 段。不可直接吸（縮殼後暈眩時可吸）；巡邏 48px/s、縮殼 190px/s。
    吞食體感修復：縮短高速段、放慢水平移動並延長暈眩窗，讓玩家可穩定讀到停下時機後長按吸入。暈眩期吸入引力 ×1.5＋判定半徑 +20%＋貼身
    ≤68px 殼殼專屬錐形豁免（停位貼腳死角）；暈眩視覺＝頭頂眩星公轉＋殼體閃白脈動（與可吸窗
...
- 數值：HP **148**、本體接觸傷害 **0.5 心**、P2 ≤70%、P3 ≤35%（PRD §6.6）、狂暴 ×1.1；地面帶
  踱步（approachPoint 以 125px/s 上限逼近擺動錨，禁座標直寫）；生存窗招（熔斷倒數/最後轉帳/
  牛熊召喚）不可暈，其餘招下砸可短暈 0.8s（§58 落地王慣例）。
- **L30 幾何與接觸安全 SSOT**：arena 只保留中央一塊更低單向平台（`offsetPx=0`、
  `y=256`、`w=280`，平台頂 248）；左右平台移除，地面仍是主路線。劉董物理箱寬高
  由 `LIUDONG.bodyHitboxWidthRatio=0.62`／`bodyHitboxHeightRatio=0.88` 統一推導，
  arena 召喚體（含每 10 HP 補給）總量上限為 1，避免小怪疊成接觸牆。
- 前室提供護盾／星力二選一；關門鎖定後由 `LevelSpec.arenaEntryGraceMs=7000`
  提供一次性讀招保護窗，涵蓋相機切換、入金演出與第一個可讀攻擊；護盾泡沿魔王關
  預設落在 P2 高風險位，讓玩家能用前段讀招修正失誤；P3 不再額外投放，仍可透過
  變身與走位處理終局（EX 不投放）。
- 變身宣告 SSOT：`bossApplies: ['gravity-form', 'transform']` 對應
  `TRANSFORM_ADVANTAGE(form='gravity', supplyFlavor='boomy')`；`formstrike` probe 以真實
  TF 輸入驗證引力井確實能命中劉董，而不是只驗證變身狀態存在。
- 每 10 HP 補生與飢荒救援的 `safeSupply` 只負責補匣／變身素材：可走位、可吸入，
  不啟動 zappy 放電、boomy 迴旋刃或本體接觸傷害；P2 牛熊召喚體不帶此標記，仍是
  玩家必須讀招處理的攻擊威脅。
...
- **招牌全屏下跌箭頭 arrowrain（P2）**：批次落下（慢大 → 快小；EX 加第三批
  ...
  中央崩跌衝擊波與雙向地面波。硬規則全數落地：每批缺口由
  `LIUDONG.arrowrainSafeLaneCount=2` 控制且恆開、錨定玩家近旁相鄰雙車道；
  全域隨機缺口在 650ms 陰影窗內跨不了 3 車道＝不公平死局，實測回調）、
  陰影預警 650ms ≥600ms。
- **P2 招池**：arrowrain／klinewave（K 線海嘯：柱列單側掃場、留玩家近旁單車道缺口）／
...
- **P3 招池**：doomarrow（末日箭：全屏箭柱雨兩波、唯一走廊白輪廓恆亮且錨定
  ...
  全場震波即核心機制）／liquidation（清算通知：8 張通知單以 `850ms` 前搖、
  `520ms` 雙發間距分批緩降，車道錯拍恆可穿行；
...
- 補給：enemyMix 以常態可吸與殼殼暈眩窗條件可吸共同供給（zappy 列首＝minionDrop
  陣列序輪替恆含雷味，shelly 走暈眩窗條件可吸）＋每損 10 HP 補生；小熊市/牛熊怪為威脅非補給（不可吸）。
- 殼殼反制實測（2026-08-02、Chromium、`l30-swallow-controls-final2`）：正確時機
  100 次中暈眩窗觀測率 76%、成功吸入率 79%，平均暈眩窗 2158ms、平均反應至吸入
  418ms；旋轉期 30 次吸入成功率 0%。兩項皆達 `AUDIT_THRESHOLDS`，console error 0；
  探測器明確以 `SHELLY_FSM.spinMs/stunMs` 與實際 state 取樣，避免把跨入暈眩的長按誤算成旋轉期成功。
- **基礎星彈 anti-softlock 契約（全遊戲收官驗收）**：全招式均有純走位解
  （缺口/走廊/縫隙錨定玩家近旁）、插針不即死、雷射站立恆安全、黑洞牽引恆低於全速；
  L30 通關率以 `scripts/level-audit.mjs` 的 high bot 5 局門檻驗收；引力化是本關
  `TRANSFORM_ADVANTAGE` 的防守 A/B（三次星體護衛＋黑洞免疫），殼化與雷化仍可作
  替代路線，不得用舊報告數字代替當前證據。2026-08-02 同口徑結果：無變身 high
  5/5（100%，TTK 平均 36.2s、通關平均 39.0s、0 死亡）、mid 2/5（40%，TTK
  平均 65.6s、通關平均 68.4s、0.4 死/run）；引力化 high 5/5（100%，TTK 平均
  39.9s、通關平均 42.7s、2.6 次變身/run、0 死亡）、mid 4/5（80%，TTK 平均
  50.4s、通關平均 53.3s、3.6 次變身/run、0.2 死/run）。四組 console error
  皆為 0；變身的可量化收益是 mid 通關率由 40% 提升至 80%，high 則提供同等通關率
  下的容錯與防守替代線。
