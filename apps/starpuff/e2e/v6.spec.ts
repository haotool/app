...
test('版本號（§42）：__sp.version 回報 semver+SHA 格式', async ({ page }) => {
...
test('L1 無技能保底通關（§43）：僅移動＋吸入＋星彈全程走完並寫檔', async ({ page }) => {
  ...
  const errors = collectErrors(page);
  await startGame(page);
  // 六輪基礎循環：吸一隻（+1 彈藥）→ 射一隻（+1 擊殺）；全程不用跳躍以上技能。
  // 負載韌性：吸入目標若被場上亂流消耗（觸碰擊退/彈開），每 3.5s 於錐內補生一隻。
  for (let round = 1; round <= 6; round += 1) {
    ...
    for (let attempt = 0; attempt < 4; attempt += 1) {
      ...
      try {
        await expect
          .poll(() => page.evaluate(() => window.__sp.ammo().ammo), { timeout: 3500 })
          .toBeGreaterThanOrEqual(1);
        break;
      } catch {
        if (attempt === 3) throw new Error(`round ${round} 吸入四次補生仍未得彈藥`);
      }
    }
    await page.keyboard.up('X');
    await page.evaluate(() => {
    ...
    await tapFire(page);
    await expect
      .poll(() => page.evaluate(() => window.__sp.quota().killCount), { timeout: 8000 })
      .toBeGreaterThanOrEqual(round);
  }
  await expect.poll(() => page.evaluate(() => window.__sp.gateOpen())).toBe(true);
