...
test('舊存檔相容（§50）：v1 四關通關存檔載入見七節點，L5 開放、L6/L7 迷霧鎖定', async ({
  ...
  // 七節點：1-4 已通關可入、5 開放可入、6/7 迷霧鎖定（無入口鈕）。
...
test('Noctra 擊破（§54）：三階段事件流走完整 FSM，勝利入結算且存檔記錄 L7', async ({ page }) => {
  ...
  // 入場運鏡完成後 BOSS_SPAWNED 設定血條（maxHp 70）。
  await expect.poll(() => page.evaluate(() => window.__sp.bossHp()), { timeout: 20000 }).toBe(70);
  ...
  await expect
    ...
    .toBeLessThan(70);
  // 正式傷害管線連段：跨 P2（≤42）與 P3（≤21）門檻直至擊破。
  await page.evaluate(() => window.__sp.damageBoss(29));
  await expect.poll(() => page.evaluate(() => window.__sp.bossHp())).toBe(40);
  await page.evaluate(() => window.__sp.damageBoss(25));
