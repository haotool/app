...
test('舊存檔相容（§64）：v9 存檔載入十二節點，L10 開放、L11/L12 迷霧鎖定', async ({ page }) => {
...
test('L10 星門折躍（§65）：跳入星門傳送至高台帶並保留可玩狀態', async ({ page }) => {
...
test('L10 保底線（§65 floorbot-no-warp）：全程地面不碰星門仍可通關', async ({ page }) => {
  test.setTimeout(150_000);
  const errors = collectErrors(page);
  await startGame(page);
  await gotoLevel(page, 10);
...
test('L11 中點重生（§66）：越過 checkpoint 後死亡自 1850 重生，進度與計時保留', async ({
  page,
}) => {
  test.setTimeout(150_000);
  const errors = collectErrors(page);
...
test('L12 魔王關體系（§67/§68）：前室增益→單向門→三階段→雙子連破寫檔含彩蛋', async ({ page }) => {
...
test('L12 保底磨王（§67）：標準傷害逐具擊破走 P3 裂核收尾', async ({ page }) => {
  test.setTimeout(180_000);
  const errors = collectErrors(page);
  await startGame(page);
  await gotoLevel(page, 12);
