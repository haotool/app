...
// 可傷窗輪詢：入場運鏡/過熱窗開啟後傷害才結算；kite＝輪詢間持續左右走位
//（P2 生存段保底行為——原地紮營吃補給怪接觸傷不是設計內玩法）。
async function pollDamage(
  ...
  while (Date.now() < deadline) {
    if (kite) {
      flip = -flip;
      const key = flip > 0 ? 'ArrowRight' : 'ArrowLeft';
    ...
    await page.waitForTimeout(kite ? 120 : 300);
  }
  return before;
}
...
test('L20 Voidra 三段（§82）：P2 生存段免傷帶、過熱窗輸出、段起點重試與全破謝幕', async ({
  ...
  await expect.poll(() => page.evaluate(() => window.__sp.playerHp())).toBe(5);
  // 過熱窗輪詢（9s/20s/33s 波次表）：窗內傷害正常結算；輪詢間持續走位（保底行為）。
  const afterOverheat = await pollDamage(page, 5, 30_000, true);
