...
// §78 真實手勢級守門：CDP dispatchTouchEvent 走真實觸控合成路徑（hit-testing＋
...
// 真實觸控派發（§78）：CDP Input.dispatchTouchEvent 走瀏覽器合成路徑，
...
// 搖桿下滑手勢：從 (150,200) 分步下滑 dy（可帶 dx 斜向），模擬拇指真實軌跡。
...
test('真實觸控：斜下 45 度滑動＝蹲住不被帶出平台，點跳穿落', async ({ page }) => {
  ...
  expect(Math.abs(x1 - x0)).toBeLessThan(30);
