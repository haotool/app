...
- 621dcc4: 行動優先 UX 極致化：貨幣切換後金額按匯率正確顯示與換算，並防止無匯率時誤算；修復小螢幕與橫向裝置的內容遮蔽，觸控目標全面達 44px；刪除等破壞性操作改用品牌化確認對話框；離線與更新橫幅不再遮擋底部操作列；已安裝的 PWA 回到前景即檢查更新並可一鍵套用新版；鍵盤操作與讀屏可達性全面補齊。
