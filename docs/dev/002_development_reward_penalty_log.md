...
> 版本：outline-v2-ultra
> 原則：每筆只保留日期、ID、原因、解法。
> 本次分數變化：-1（reward 0、penalty 1、neutral 0）｜累計總分：+314
...
- 原因：audit driver 的變身策略在 #947／#953 之後退化為死迴圈——空匣才補星、只補剛好 3 顆、資格成立即無條件變身，而變身正好扣 3 顆，遂形成「補 3 → 變身 → 扣光 → 空匣 → 再補 3」；實測形態 uptime 逼近 100%（56 次/600s、形態長 10s），引力化無遠程手段故全程零輸出，L28 六輪全 timeout、TTK 從未達成
- 解法：補星改補到滿匣（變身扣 3 後留餘彈，據以驗證 #947 變身期可射星）、形態落幕後設 6s 再變身冷卻（uptime 約 62%）、持有彈藥時不主動變身。根因是量測工具的行為建模未隨玩法契約更新——同類風險在 #952 拆鍵時已發生一次（driver 仍按舊鍵），故 driver 應列入玩法契約變更的同步清單

- 日期：2026-07-31
- ID：neutral-starpuff-mix-counts-toward-transform
- 原因：合成為自動觸發（素頂槽遇配方夥伴即合成），會把兩顆星轉成對變身資格毫無貢獻的槽——等於系統無預警沒收累積，與 953 放寬資格的意圖直接衝突；玩家為了湊變身反而要刻意避開合成，而合成本身是升級不該有此代價
- 解法：合成槽改計入其配方 pair 的兩個來源味各 1 單位（getMix(pair) 反查，非新增第二份映射）；consumeForTransform 配合改兩段扣除——素星優先、合成星保底，避免高價值槽被優先吃掉，同時確保「資格成立必扣得滿」不留死結。金星維持不計入以與合成星區辨

- 日期：2026-07-31
- ID：neutral-starpuff-starburst-manual-and-dynamic
- 原因：星暴固定 12 魔王傷、成本卻是整匣 5 槽，而同樣 5 槽直射有 25（素星）至 40（強化星）——魔王戰兌換率明確虧損；且結晶為滿匣自動觸發，玩家對「累積被沒收」沒有選擇權。副作用是彈匣永遠停不在 5 槽，使 swallowIntoMagazine 的滿匣替換分支近乎不可達（該分支 #947 已正確實作卻無從觸發）
- 解法：結晶改 SP 手動觸發（canCrystallize 取代 shouldCrystallize），滿匣遂成穩定狀態、替換分支自然可達；魔王傷害改 starstormBossDamage 依投入星值 ×1.2 計價並保底 12，於結晶瞬間封存進相位（彈匣隨即清空，引爆時已無從回讀），跨關持有一併帶走。走動關不受影響——該場景價值來自全屏清場與 5 秒無敵。SP 承載結晶與引爆兩義但天然互斥（結晶後即進 charged），每個狀態仍單義，不回退 #952 的重疊歧義

- 日期：2026-07-31
- ID：neutral-starpuff-relax-transform-eligibility
- 原因：eligibleForm 逐槽檢查「異味／金星／合成星即 return null」，門檻寫 3 星實際要求整匣零雜質——3 迴旋味＋2 果凍味不成立，玩家遂感受為「非得連吞」；地面限制另散落三處（裁決／主圖示／次要徽記），任一漏改即出現「圖示說可以、按下去沒反應」
- 解法：改為逐味統計星單位取達標且已解鎖者，金星／合成星改為不計入但不否決；並列以「單位最多、同數取最早出現」決定性裁決。連動 consumeForTransform 必須只扣中選味系（修前無差別自底部扣，放寬後會吃掉無關星），該函式修前零測試覆蓋，本次補 5 案。地面限制隨 airborne 參數整個移除，以 fn.length 不變式釘住「不再是輸入」而非依賴分支
...
- 原因：formstrike 探針掃過四個形態驗收魔王關（L22/L24/L26/L28）後確認無新的零傷害病灶，但量出潮化水引單發 2 傷／0.7s CD ＝ 2.9 DPS，是四形態技唯一異常值（焰彈 12.5、引力井 11.7、稜片 21.4）——對 108+ HP 魔王需 54 次施放約 38 秒，而形態期只有 10 秒，宣告的「優勢解」名不副實
- 解法：沿引力井逐跳模式補 3 跳滯留段（2×(1+3)=8 傷／0.7s ＝ 11.4 DPS），刻意對齊同儕而非取稜片的 21.4——潮化定位在牽引控場不該同時是最高輸出；牽引仍只起手結算一次，供給型「只拉不傷」豁免沿用。probe 複驗中位傷害 2→8 恰為設計值。過程中先誤判「四形態技全部過弱」，是把單次施放傷害當成 DPS——ember/prism 為高頻投射物，DPS 本就健康，故未動
...
- 原因：雙席 APPROVE 後 Should-fix 收尾——缺 patch changeset（碰撞箱穩定屬使用者可感知修復）、五王 destroy 未防禦性 unregister、L16 EX 樣本 n=3 不足契約、GAME_DESIGN 路徑/退場標註兩處 nit
- 解法：補玩家視角 patch changeset、五王 destroy 補 vscale.unregister（比照 player）、L16 EX high 加樣 n=6（6/6 通關、TTK 43.3s、2.0 死/run，合併 n=9 對 base 無漂移）、§77.1 加已退場標註＋§117 路徑修正
...
- 原因：#816 變身優勢「存在但不可量測」——每王優勢情境無資料模板、bot 無變身能力，TTK 差門檻（≥15%）無法裁定
- 解法：TRANSFORM_ADVANTAGE 模板落 difficulty.ts（Jellord 殼化反彈線/Noctra 雷化斷召線，supplyFlavor→form 由 eligibleForm 守門零漂移）＋level-audit --transform bot hook（集星停火→SP 變身→transformsPerRun 報告），GAME_DESIGN §110 回寫＋minor changeset
