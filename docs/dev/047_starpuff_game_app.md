...
- 遊戲 sprite 為 WebP（q82，透明保留），置於 `src/assets/sprites/`，由 `core/assets.ts` manifest 統一載入；PWA/OG 圖（icon-192/512 PNG+SVG、maskable、favicon、og-image）置於 `public/icons/`，生成紀錄見 epic 的 art-log。
- 音效由 ZzFX 參數即時合成，repo 不存音訊檔。
- Workbox `globPatterns` 涵蓋 `js/css/html/svg/png/webp/woff2`，單檔上限 5 MiB；新增大型資產前先確認 precache 體積。
- SW 採 `autoUpdate`：單 bundle 遊戲撕裂風險低，屬已評估接受之風險。
