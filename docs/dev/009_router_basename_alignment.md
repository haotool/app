...
FAQ / About 直接以 `https://app.haotool.org/faq` 或 `pnpm preview` 預覽的 `/faq`、`/about` 會顯示空白頁，因為 React Router `BrowserRouter` 被強制設定 `basename='/ratewise'`，但 Vite build 預設 `base='/'`。部署或預覽若忘記覆寫 `VITE_BASE_PATH`，路由與靜態資產的根路徑就不同步，導致非 `/ratewise/*` URL 無法 hydrate。此問題同時影響 SEO（搜尋引擎索引的是 `/faq` 但實際內容在 `/ratewise/faq`）。
...
3. **會破壞什麼嗎？**
   - 若只在 Router 端改寫仍需人工輸入 `VITE_BASE_PATH`，容易再次遺漏。因此額外建立 `.env.production`，讓 build / preview 預設就是 `/ratewise/`。經 `pnpm typecheck`、`pnpm test`、`pnpm build:ratewise`、`pnpm preview` 驗證後，舊功能無回歸。
...
- Zeabur 仍以 `/ratewise/` 子路徑曝光，所以 sitemap、canonical、PWA manifest 都需落在同一層；把 `VITE_BASE_PATH` 固定寫進 `.env.production` 可以避免遺漏，並保留在開發模式自動 fall back 至 `/`。
...
2. **生產環境 base 設定**
   - 在 `apps/ratewise/.env.production` 建立 `VITE_BASE_PATH=/ratewise/`。`pnpm build:ratewise` 會自動載入該檔，Vite `base` 與 Router `basename` 與 SEO URL 因此保持一致。
3. **文件與流程**
