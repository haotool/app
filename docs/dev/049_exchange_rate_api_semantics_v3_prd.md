...
> **本規則為條件式，非本專案現況**：§15.1 已查證台銀與明洞換錢所在牌告換現鈔情境下**皆不收手續費**，故 `feeCoverage: "complete"`、`allInReceivedAmount` **可計算**。此處保留規則是為了處理未來新增有費用 provider 的情況。
...
## 14. 本產品是什麼／不是什麼（v2 定稿）

> **重新定稿原因**：初版以「無費用資料」作為不能提供 all-in 金額的理由，該前提已被 §15.1 的 `fee = 0`（已查證、`coverage: complete`）推翻。事實上本產品**可以**在界定情境內給出 all-in 金額——真正的限制不是「算不出來」，而是「算得出成本結構 ≠ 保證你拿得到」。

### 14.1 定位

> **RateWise 是「TWD↔KRW 的台銀與明洞換錢所牌告價比較資料服務」，外加台銀多幣別牌告匯率查詢。**

### 14.2 是什麼

| 提供                             | 範圍                                                   |
| -------------------------------- | ------------------------------------------------------ |
| TWD↔KRW 的跨 provider 牌告價比較 | 台銀 vs 明洞換錢所（**唯一**可比對，§13.3.4）          |
| 多幣別台銀牌告匯率查詢           | 現金／即期各買入賣出四種報價                           |
| **界定情境內的 all-in 實得金額** | 牌告匯率 × 現金換匯 × 無手續費（§15.1 `pricingScope`） |

> **為何 all-in 可以給**：已查證台銀與明洞換錢所在此情境下**皆不收手續費**，價差即全部成本。因此 `feeCoverage: "complete"`、`fee.amount: 0`，`allInReceivedAmount` **可計算**而非恆 `null`。

### 14.3 不是什麼

| 不是                       | 原因                                                                                                                            |
| -------------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| 全球匯率比較服務           | 計價基準不同，16 個幣別代碼交集**不可比**；唯一可比對為 TWD↔KRW（§13.3.4）                                                      |
| **現場成交保證**           | 牌告價會變動；**現場庫存、營業時間、身分證件要求**皆不在資料範圍（`quoteAvailability: "indicative_not_transaction_guarantee"`） |
| **牌告情境以外的報價來源** | **線上結匯優惠匯率**、旅行支票、大額預約已由 `pricingScope.excludedConditions` 排除；顯示數字**不適用**於這些管道               |
| 市場中價的權威來源         | `derivedQuoteMidpoint` 是我方兩側牌告的算術中點（`isMarketRate: false`）；外部市場中價僅為**對照組**且授權未確認（§17 #1）      |
| 投資或交易建議             | 資料僅供參考，不含即時性保證                                                                                                    |

### 14.4 all-in 與「保證」的分界

這兩件事必須分開，否則不是低估自己就是過度承諾：

| 可以說                                       | 不可以說                      |
| -------------------------------------------- | ----------------------------- |
| 「在牌告匯率換現鈔情境下，價差就是全部成本」 | 「保證你換得到這個金額」      |
| 「依此牌告價，30,000 TWD 可換 N 韓元」       | 「你到現場一定拿到 N 韓元」   |
| `feeCoverage: "complete"` + `fee.amount: 0`  | 隱去 `quoteAvailability` 限定 |

> **第三列的排除項是本次稽核的直接產物**：§20.1 發現人民幣 FAQ 寫「線上結匯（如台銀 Easy 購）匯率最優惠」，而該管道正是我方 `pricingScope` 明確排除的——文案推薦了一個我方數字不適用的選項。定位文件必須讓這種矛盾**可被檢出**。

> 定位精確不是退讓，而是**權威性的前提**。§13.4 第 10 項「中立性與責任界面」要求說清楚責任邊界；宣稱超出實際覆蓋範圍會失去權威，**低估自己能力**同樣會——初版就低估了。
