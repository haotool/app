...
> **Status (2026-05-10)：Superseded / rebased.** 本 plan 的歷史資料與公開 API 合約已由
> `2026-05-10-canonical-provider-rate-api.md` 接管；MoneyBox 路徑統一改為 provider canonical
> path，不再保留 `moneybox.json` / `moneybox-history` alias。
...
- [x] **Task 1: MoneyBox 歷史快照**
      `update-moneybox-rates.yml` 每次 MoneyBox 匯率變更時，除了 `public/rates/providers/moneybox/latest.json`，也保存 `public/rates/providers/moneybox/history/YYYY-MM-DD.json`，並讓 data branch refresh 與 CDN purge 覆蓋該路徑。
