#!/usr/bin/env node
/**
 * experiment/ratewise-ux-2026 @ e5bc0d90 — 5 工程師並行 QA 稽核腳本
 * Usage: AUDIT_BASE_URL=http://127.0.0.1:4173/ratewise node scripts/qa-experiment-e5bc0d90.mjs
 */
...
const TAG = process.env.AUDIT_TAG ?? 'e5bc0d90';
...
async function auditPage(page, route, viewport, engineerId) {
  ...
  const audit = await page.evaluate(() => {
    ...
    const heroRateEl = document.querySelector(
      '[data-testid="hero-rate-display"], [data-testid="hero-rate"], [data-testid="sell-rate"], [data-testid="hero-v2-rate"], .hero-rate',
    );
    if (heroRateEl) {
      ...
      for (const el of document.querySelectorAll('span, div, p, strong')) {
        ...
        if (/^\d[\d,.]+$/.test(t) && t.length >= 2) {
          ...
          const parent = el.closest(
            '[class*="rate"], [data-testid*="rate"], [data-testid*="hero"]',
          );
          if (fs >= 18 && parent) {
    ...
    const heroV2El = document.querySelector(
      '[data-testid="hero-v2"], [data-testid="hero-v2-rate"], [data-hero-layout="v2"]',
    );
    const heroV2Default = !!heroV2El;
    ...
    const stickyCta = fixedBottom.filter(
      (f) =>
        /換算|匯率|立即|開始|CTA/i.test(f.text) || f.text.includes('換') || f.text.includes('算'),
    );

    return {
      truncation,
      fixedBottom,
      heroInfo: { title, h1, sellRateY, sellRateFontSize, heroV2Default },
  ...
  const shotName = `qa-${engineerId}-${TAG}-${route.page}-${viewport.name}.png`;
  ...
  return {
    ...
    ac: {
      ...
      'AC-HERO-DEFAULT': audit.heroInfo.heroV2Default ? 'pass' : 'fail',
      'AC-TOUCH-01': bottomClipped.length === 0 ? 'pass' : 'fail',
      'AC-TOUCH-02': touchSmall.length === 0 ? 'pass' : 'fail',
      'AC-LAND-01':
        isLanding && audit.heroInfo.sellRateY !== null && audit.heroInfo.sellRateY < viewport.height
          ? 'pass'
          : isLanding
            ? 'fail'
            : 'n/a',
      'AC-LAND-02': isLanding && audit.stickyCta.length > 0 ? 'pass' : isLanding ? 'warn' : 'n/a',
...
function buildGateChecklist(matrix390, allResults) {
  const console12 = matrix390.every((m) => m.console === 0);
  const no418 = matrix390.every((m) => !m.has418);
  const homeHero = allResults.find((r) => r.engineer === 'E2' && r.page === 'home');
  const heroDefault = homeHero?.heroInfo?.heroV2Default ?? false;
  const heroPass = homeHero?.ac?.['AC-HERO-01'] === 'pass';
  const heroFont = homeHero?.ac?.['AC-HERO-02'] === 'pass';
  const landSticky = ['usd-twd', 'jpy-twd', 'twd-jpy'].every((p) => {
    const r = allResults.find(
      (x) => x.engineer === 'E1' && x.page === p && x.viewport === '390x844',
    );
    return (r?.stickyCta?.length ?? 0) > 0 || (r?.fixedBottom?.length ?? 0) > 0;
  });
  const multiTouch = matrix390.find((m) => m.page === 'multi');
  const settingsClip = matrix390.find((m) => m.page === 'settings');

  return {
    'AC-CON-01 (12/12 console=0)': console12 && no418 ? 'pass' : 'fail',
    'AC-HERO default hero-v2 (no ?ux=)': heroDefault && heroPass && heroFont ? 'pass' : 'fail',
    'AC-LAND sticky CTA (3 landing)': landSticky ? 'pass' : 'warn',
    'AC-TOUCH multi (390 clip=0)': (multiTouch?.clip ?? 99) === 0 ? 'pass' : 'fail',
    'AC-TOUCH settings (e3 clip=0)': (settingsClip?.e3clip ?? 99) === 0 ? 'pass' : 'fail',
    'AC-PWA no modal on load': allResults.every((r) => !r.pwaModalVisible) ? 'pass' : 'fail',
  };
}
...
async function main() {
  ...
  for (const eng of ENGINEERS) {
    for (const vp of eng.viewports) {
      ...
      const routeList = ROUTES_12.filter((r) => eng.routes.includes(r.page));
  ...
  const matrix390 = ROUTES_12.map((route) => {
    ...
    const e2 = allResults.filter((x) => x.engineer === 'E2' && x.page === 'home');
    const e1 = allResults.find(
      (x) => x.engineer === 'E1' && x.page === route.page && x.viewport === '390x844',
    );
    return {
      route: route.path,
      page: route.page,
      ...
      heroV2: r?.heroInfo?.heroV2Default ?? e2.some((x) => x.heroInfo?.heroV2Default),
      stickyCta: e1?.stickyCta?.length ?? 0,
      ...
      gate: {
        con: r?.ac?.['AC-CON-01'] ?? '?',
        hero: r?.ac?.['AC-HERO-01'] ?? 'n/a',
        touch: r?.ac?.['AC-TOUCH-02'] ?? '?',
      },
    };
  });

  const gateChecklist = buildGateChecklist(matrix390, allResults);
  ...
  const out = {
    ...
    summary: {
      ...
      matrixConsoleZero: matrix390.filter((m) => m.console === 0).length,
    },
  };
  ...
  console.log(
    JSON.stringify({ jsonPath, summary: out.summary, gateChecklist, matrix390 }, null, 2),
  );
}
