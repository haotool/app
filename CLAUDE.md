...
- `pre-commit` 第 6 步（`scripts/verify-002-log.mjs`，**無條件執行**，跳過與否由腳本自行判定）自動驗證記分：`a+b+c` = 本次新增條目數、`N = a - b`、`T` = 前版累計 + `N`、四行模板、歷史條目不可刪除（issue #608）；staged 刪除整份 002（`git rm`）亦必紅。ID 唯一性只對**本次造成的重複**擋 commit（歷史既有重複不回溯，否則會卡死所有後續 commit）；刪除比對只採信基準版解析結果，區段外的獨立 `- ID：` 行不列入。**基準版不可解析、或讀不出前版累計總分時一律 fail-closed**（否則先 poison tip 再清空歷史即可全綠）；刪除比對另以原始文字掃描 `- ID：` 作第二道保險。
