...
- `pre-commit` 第 6 步（`scripts/verify-002-log.mjs`，**無條件執行**，跳過與否由腳本自行判定）自動驗證記分：`a+b+c` = 本次新增條目數、`N = a - b`、`T` = 前版累計 + `N`、四行模板、ID 唯一性與歷史條目不可刪除（issue #608）；staged 刪除整份 002（`git rm`）亦必紅。**基準版不可解析、或讀不出前版累計總分時一律 fail-closed**（否則先 poison tip 再清空歷史即可全綠）；刪除比對另以原始文字掃描 `- ID：` 作第二道保險。
