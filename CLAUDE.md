...
**匯率 API v3 發布**：`scripts/publish-fx-release.mjs` 先以內容雜湊驗證 immutable provider/history objects，再原子替換 `public/rates/v3/current.json`；`update-latest-rates.yml` 與 `update-moneybox-rates.yml` 必須從觸發 SHA checkout 完整 publisher 依賴，只有 `RATEWISE_FX_V3_ENABLED=true` 才推進 v3 pointer。啟用 gate 後單一 provider 抓取失敗仍發布 verified carry-forward/failed status，legacy latest 不得被失敗資料覆寫。v3 current 是 mutable pointer，data branch push 後必須一併 purge current URL；immutable objects 不需 purge。前端以 `apps/shared/fx/release.ts` 驗證 current → manifest → latest snapshot 鏈，歷史資料另行驗證，不得把 v2 compatibility projection 當成 v3 release receipt。
