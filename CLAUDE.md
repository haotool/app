...
- **Cloudflare Pages**：靜態 8-app 產物由 `scripts/build-pages.mjs` 組裝，GitHub Actions 以 Wrangler Direct Upload 部署 `haotool-static`；正式網域仍由 `security-headers` Worker 控制，未完成 preview／SEO／PWA／header／API 驗證前不得切換 `STATIC_ORIGIN`
